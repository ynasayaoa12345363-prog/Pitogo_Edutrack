...
function sendOtpEmail($toEmail, $parentName, $otp, $smtpEmail, $smtpAppPassword, $smtpFromName): bool {
        ...
        $mail->Body = "
            <div style='font-family:Arial,sans-serif;background:#f4f7f9;padding:30px;'>
                <div style='max-width:540px;margin:auto;background:white;padding:32px;border-radius:18px;text-align:center;border:1px solid #e2e8f0;'>
                    ...
                    <p style='color:#64748B;'>Hello <b>" . htmlspecialchars($parentName) . "</b>, use this OTP to verify your password change.</p>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'request_password_otp') {
        ...
        $stmt = $pdo->prepare("SELECT first_name, middle_name, last_name, email, password FROM users WHERE id = ? AND role = 'parent' LIMIT 1");
        ...
        if (sendOtpEmail($user['email'], $parentName, $otp, $smtpEmail, $smtpAppPassword, $smtpFromName)) {
            echo json_encode([
                'success' => true,
    ...
    if ($action === 'verify_password_otp') {
        ...
        logAudit($pdo, $currentUserId, 'Changed password from parent settings');
...
$stmt = $pdo->prepare("SELECT * FROM users WHERE id = ? AND role = 'parent' LIMIT 1");
$stmt->execute([$currentUserId]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);
...
$parentName = fullName($user);
$avatarUrl = getAvatar($user, $parentName);
