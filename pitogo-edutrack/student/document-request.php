...
/*
|--------------------------------------------------------------------------
| Pitogo EduTrack - Student/Parent Document Request
|--------------------------------------------------------------------------
| Save as:
...
if (!in_array($currentRole, ['student', 'parent'], true)) {
...
$roleLabel = $currentRole === 'parent' ? 'Parent Portal' : 'Student Portal';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_otp'])) {
    ...
    if ($requestData) {
        ...
        logAudit(
            ...
            $currentUserId,
            $currentRole,
            'Opened secure document using OTP',
            'document_requests',
            $requestId
        );
...
<section class="content-area">
    ...
    <?php if ($currentRole === 'parent' && !$student): ?>
        <div class="alert danger">
            <i class="fa fa-triangle-exclamation"></i>
            No linked student account found. Please use a valid parent access code first.
        </div>
    <?php endif; ?>
    ...
    <div class="dashboard-grid">
        <div class="panel">
            ...
            <form method="POST">
                ...
                <?php if ($currentRole === 'parent' && $linkedStudents): ?>
                    <div class="form-group">
                        <label>Select Child</label>
                        <select name="student_id" class="form-control" onchange="window.location.href='document-request.php?student_id=' + this.value;">
                            <?php foreach ($linkedStudents as $linked): ?>
                                <?php $linkedName = fullName($linked); ?>
                                <option value="<?= e($linked['id']) ?>" <?= (int)$linked['id'] === (int)$studentId ? 'selected' : '' ?>>
                                    <?= e($linkedName) ?><?= !empty($linked['id_num']) ? ' - ' . e($linked['id_num']) : '' ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
